...
% % Apply correction when necessary
% for n = [14 13 12 11 10 7]
%     runs = fieldnames(Permeameter.(char(ID(n))));
%     n
% 
%     for m = 2:length(runs)
%         m
%         qqqqqq = mean(Permeameter.(char(ID(n))).(char(runs(m))).Flow_Rate);
%         kkkkkk = mean(Permeameter.(char(ID(n))).(char(runs(m))).Permeability);
%         p = polyfit(qqqqqq,1/kkkkkk,1);
%         yintercept = 1/p(2)
%         % Permeameter.(char(ID(n))).(char(runs(m))).Permeability = k;
%     end
% end
...
DataApril25 = table2array(Data_April2025(:,2:7));
...
errorbar(DataApril25(:,2),DataApril25(:,1),DataApril25(:,6),DataApril25(:,5),DataApril25(:,4),DataApril25(:,3),'.')
plot(DataApril25([1,3],2),(DataApril25([1,3],1)),'ok','MarkerSize',sz)
plot(DataApril25([2,4],2),(DataApril25([2,4],1)),'vr','MarkerSize',sz)

for n = 1:size(Data)
    errorbar(Por(n),(Perm(n)),(yneg(n)),(ypos(n)),xneg(n),xpos(n),'.')
    s = char(Data.ID(n));
    if s(1:7) == "CC23_2a";
        plot(Por(n),(Perm(n)),'dr','MarkerSize',sz)
    elseif s(1:6) == "CC23_2";
        plot(Por(n),(Perm(n)),'dk','MarkerSize',sz)
    elseif s(1:6) == "CC23_4";
        plot(Por(n),(Perm(n)),'or','MarkerSize',sz)
    elseif s(1:6) == "LV23_1";
        plot(Por(n),(Perm(n)),'vk','MarkerSize',sz)
    elseif s(1:6) == "LV23_2";
        plot(Por(n),(Perm(n)),'^k','MarkerSize',sz)
    elseif s(1:6) == "Enclav";
        plot(Por(n),(Perm(n)),'sk','MarkerSize',sz)
    end
...
YY = Perm';
YY = YY(1:11);
scatter([1 1 1], (YY(1:3)),'dk','jitter','on','JitterAmount',0.1);
scatter([2 2 2], (YY(4:6)),'ok','jitter','on','JitterAmount',0.1);
scatter([4 4 4], (YY(7:9)),'vk','jitter','on','JitterAmount',0.1);
scatter([3 3], (YY(10:11)),'^k','jitter','on','JitterAmount',0.1);
